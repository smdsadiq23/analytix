...
def get_detail_so(so_name):
    ...
    # Fetch SO header and item-level details
    ...
    # Get size-wise quantities from SO Items
    ...
    # Fetch all relevant scan logs for this SO
    ...
    # Aggregate in Python
    metrics = defaultdict(lambda: {"completed": 0, "rejected": 0})
    ...
    # Build final list
    metrics_by_op = []
    operations = {log.operation for log in scan_logs}
    ...
    for op in operations:
        for size in sizes:
            ...
            rejected = metrics[key]["rejected"]
            pending = max(size_qty - completed - rejected, 0)
...
def get_detail_wo(wo_name):
    ...
    # Get size-wise quantities from WO Line Items
    size_qty_list = frappe.db.sql("""

...
def get_order_map(filters):
    """Order qty, delivery date, season — keyed by (style, colour, size)."""
    conditions = []
    params     = {}
...
def get_daily_production(filters):
    ...
    Grouped by date / operator / style / colour / size.
...
def get_data(filters):
    ...
    for log in daily_logs:
        ...
        # Yield % = (Actual Wt / Planned Wt) × 100
        # > 100% means more yarn was consumed than planned
        yield_pct = round((actual_weight / planned_weight) * 100, 2) if planned_weight else 0.0
        yield_pct_str = f"{yield_pct:.1f}%"

        # Wastage/Excess % = ((Planned Wt - Actual Wt) / Actual Wt) × 100
        # Positive = saved material (good, green), Negative = over-used (bad, red)
        wastage_excess = round(((actual_weight - planned_weight) / planned_weight) * 100, 2) if actual_weight else 0.0

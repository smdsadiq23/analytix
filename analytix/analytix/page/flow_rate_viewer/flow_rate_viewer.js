...
frappe.pages["flow-rate-viewer"].on_page_load = function (wrapper) {
	...
	async function renderTen() {
		...
		const labels = ten.map((r) => r.label || "");
		const output = ten.map((r) => Number(r.output || 0));
		const target = ten.map((r) => Number(r.target || 0));
		const avg = ten.map((r) => Number(r.avg_output || 0));

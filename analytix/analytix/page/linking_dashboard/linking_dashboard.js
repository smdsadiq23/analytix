frappe.pages["linking-dashboard"].on_page_load = function (wrapper) {
	frappe.ui.make_app_page({
		parent: wrapper,
		title: "Linking Dashboard",
		single_column: true,
	});
		...
		<div class="lkd-root">
			<div class="lkd-topbar">
				<div class="lkd-brand">
					...
					<div>
						<div class="lkd-brand-title">Linking Dashboard</div>
						<div class="lkd-brand-sub">Real-time Linking Cell Overview</div>
					</div>
				</div>
				<div class="lkd-clock">
					<div id="lkd-time">--:-- --</div>
				...
				<table class="lkd-table">
					<thead>
						<tr class="lkd-head">
							...
							<th class="th-sizes">LINKING<br><span class="th-inout">IN / OUT per size</span></th>
...
frappe.pages["linking-dashboard"].on_page_show = function (wrapper) {
	$("header.navbar").hide();
	$(".page-body").css({ "padding": "0", "margin": "0" });
	$(".layout-main-section-wrapper").css({ "padding": "0", "margin": "0" });
	$(".layout-main-section").css({ "padding": "0", "margin": "0", "max-width": "100%" });
...
frappe.pages["linking-dashboard"].on_page_hide = function () {
	$("header.navbar").show();
...
const SCROLL_CONFIG = { step: 82, interval: 5000, pauseOnHover: true, edgePause: 2000 };
...
function _load() {
	frappe.call({
		method: "analytix.analytix.page.linking_dashboard.linking_dashboard.get_dashboard_data",
		freeze: false,
		callback: function (r) {
...
function _render(rows) {
	if (!rows.length) { _setState("No linking data found."); return; }
	var html = "";
	rows.forEach(function (r) {
		...
		// ── Per-size grid (sizes differ per style) ────────────────────────
		...
		// ── Total Completion (numeric) ────────────────────────────────────
		var totalOut = r.total_linking_out || 0;
		html += '<td class="td-total">' + _n(totalOut) + "</td>";

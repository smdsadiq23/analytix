...
frappe.pages["output-target-viewer"].on_page_load = function (wrapper) {
  ...
  `).appendTo($root);
	...
	function addClearAll(control, fieldname, isMulti = false) {
		const $host = control.$wrapper.find(".control-input, .control-input-wrapper").first()
			.length
		...
		if (!$btn.length) {
			$btn = $(
				`<button type="button" class="kpi-clear-btn" data-for="${fieldname}" title="Clear">×</button>`
			)
				.appendTo($host)
				.on("click", (e) => {
					e.preventDefault();
					...
					if (isMulti) {
						...
						try {
							control.set_input && control.set_input("");
						} catch {}
						try {
							control.$input && control.$input.val("");
						} catch {}
					}
		...
		control.$input &&
			control.$input.on("input change blur awesomplete-selectcomplete", toggle);
